...
async function getJsFilesRecursive(dir) {
  ...
  const nested = await Promise.all(
    entries.map(async (entry) => {
      ...
      if (entry.isDirectory()) {
        return getJsFilesRecursive(fullPath);
      }
...
async function buildPublicAssets() {
  ...
  const jsFiles = await getJsFilesRecursive(srcPublicDir);
  await Promise.all(
    jsFiles.map((srcPath) => minifyJsFile(srcPath, path.join(outPublicDir, path.relative(srcPublicDir, srcPath))))
  );
}
